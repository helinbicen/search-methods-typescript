class PuzzleNode {
    ...
    constructor(state: number[], parent: PuzzleNode | null, action: string, depth: number) {
      this.state = state;
      this.parent = parent;
      this.action = action;
      this.depth = depth;
  ...
  function dfsLimitedIterations(startState: number[], goalState: number[], maxIterations: number): PuzzleNode | null {
